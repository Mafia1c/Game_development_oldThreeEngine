...
GodShieldOBJ.NPC = 388
GodShieldOBJ.cfg = SL:Require("GUILayout/config/TalismanCfg",true)
GodShieldOBJ.attr_str = {
    [1] = "功魔道: %s-%s",
    [2] = "防魔御: %s-%s",
    [3] = "生命加成: %s%%",
    [4] = "次元之力: %s",
}
GodShieldOBJ.tips_str = {
    "解封<[异界:次元神盾]/FCOLOR=250>，需佩戴<满觉醒[宗师神盾]/FCOLOR=250>，使用<[次元魔石]/FCOLOR=250>唤醒异界之力",
    "佩戴<[异界:次元神盾]/FCOLOR=250>，在<[异界地图]/FCOLOR=250>中，每<[击杀1只]/FCOLOR=250>怪物，可获得<[1点成长值]/FCOLOR=250>",
    "异界:次元之盾<[成长值累计满]/FCOLOR=250>后，可使用<[次元魔石]/FCOLOR=250>进行次元属性唤醒",
}

function GodShieldOBJ:main(count, value)
	...
	-- Create EquipShow_1
	local EquipShow_1 = GUI:EquipShow_Create(self.ui["FrameLayout"], "EquipShow_1", 450, 320, 16, false, {bgVisible = false, doubleTakeOff = false, look = true, movable = false, starLv = true, lookPlayer = false, showModelEffect = false})
    ...
    GUI:Image_loadTexture(self.ui.bg_Image, "res/custom/npc/90cy/bg11.png")
end

function GodShieldOBJ:initClickEvent()
    ...
    GUI:addOnClickEvent(self.ui.startBtn, function()
        if self.cur_wakeUp_count <= 0 then
            SendMsgCallFunByNpc(GodShieldOBJ.NPC, "TalismanNpc", "onClickUnseal", GodShieldOBJ.NPC)           -- 解封
        else
            SendMsgCallFunByNpc(GodShieldOBJ.NPC, "TalismanNpc", "onClickWakeUp", GodShieldOBJ.NPC)           -- 唤醒
        end
    end)
end

function GodShieldOBJ:showUiInfo()
    ...
    if self.cur_wakeUp_count > 0 then
        ...
        for k, v in ipairs(self.attr_str) do
            ...
            GUI:RichTextFCOLOR_Create(self.ui["attrList"], "_attr_left_"..k, 28, y, str_1, 200, 18, "#ffffff")
            ...
            GUI:RichTextFCOLOR_Create(self.ui["nextList"], "_attr_right_"..k, 28, y, str_2, 200, 18, "#00ff00")
            y = y - 25
        end
...
function GodShieldOBJ:flushView(count, value)
...
local function onClickNpc(npc_info)
    if npc_info.index == GodShieldOBJ.NPC then
