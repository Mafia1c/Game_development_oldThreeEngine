local TianMingAwakeningOBJ = {}
TianMingAwakeningOBJ.Name = "TianMingAwakeningOBJ"
local __cfg = SL:Require("GUILayout/config/TianMingAwakeningCfg",true)
local tmp_tab = {}
for k, v in ipairs(__cfg) do
    tmp_tab[v.num] = tmp_tab[v.num] or {}
    table.insert(tmp_tab[v.num], v)
end
TianMingAwakeningOBJ.cfg = tmp_tab
TianMingAwakeningOBJ.RunAction = true
TianMingAwakeningOBJ.NPC = 291
TianMingAwakeningOBJ.effect_pos = {
    [1] = {x = -45, y = -7},
    [2] = {x = -64, y = -46},
    [3] = {x = -7, y = -54},
    [4] = {x = 27, y = -55},
...
TianMingAwakeningOBJ.rewardData = {
    [1] = 51029,
    [2] = 51030,
    [3] = 51031
}

function TianMingAwakeningOBJ:main(sMsg)
    local parent = GUI:Win_Create(self.Name, 0, 0, 0, 0, false, false, true, false)
    self._parent = parent
    GUI:LoadExport(parent, "npc/TianmingAwakeningUI", function () end)
    ...
    GUI:ItemShow_updateItem(self.ui["needItem_3"], {index = 0, look = true, bgVisible = true, count = 0, color = 255})
    GUI:Text_setString(self.ui.successRate, "")
end

function TianMingAwakeningOBJ:initClickEvent()
    ...
    GUI:addOnClickEvent(self.ui.AwakeningBtn, function()
        ...
        SendMsgCallFunByNpc(TianMingAwakeningOBJ.NPC, "TianMingAwakeningNpc", "onClickAwakening", self.cur_select_index.."#"..cur_level)
    end)
    ...
    GUI:CheckBox_addOnEvent(self.ui.CheckBox_1, function (sender)
        ...
        SendMsgCallFunByNpc(TianMingAwakeningOBJ.NPC, "TianMingAwakeningNpc", "onChangeSelectedState", param)
    end)
end

function TianMingAwakeningOBJ:updateNeedItem(i)
    local cur_level = self.level_tab[i]
    local _cfg = self.cfg[cur_level][i]
    local color1 = 255
    ...
    local color3 = 255
    local item_id3 = SL:GetMetaValue("ITEM_INDEX_BY_NAME", _cfg.needitem3)
    local needValue3 = _cfg.itemnum3
    if tonumber(SL:GetMetaValue("MONEY_ASSOCIATED", item_id3)) < needValue3 then
        color3 = 249
    end
    GUI:ItemShow_updateItem(self.ui["needItem_3"], {index = item_id3, look = true, bgVisible = true, count = needValue3, color = color3})  
end

function TianMingAwakeningOBJ:showRootBoneEffect()
...
function TianMingAwakeningOBJ:flushView(sMsg)
...
function TianMingAwakeningOBJ:showRewardItem()
    local low_level = 3
...
local function onClickNpc(npc_info)
    if npc_info.index == TianMingAwakeningOBJ.NPC then
        SendMsgClickNpc(npc_info.index.."#TianMingAwakeningNpc")
    end
end
SL:RegisterLUAEvent(LUA_EVENT_TALKTONPC, "TianMingAwakeningOBJ", onClickNpc)

return TianMingAwakeningOBJ