...
function PlayerSuperEquip.main(data)
    ...
    -- 初始化自定义按钮事件
    PlayerSuperEquip.InitOtherBtnEvent()
    return true
end
...
--#region 自定义按钮事件
function PlayerSuperEquip.InitOtherBtnEvent()
    local luckEquipBtn = PlayerSuperEquip._ui.luckEquipBtn --#region 福装备
    --#region 福装备
    if luckEquipBtn then
        GUI:addOnClickEvent(luckEquipBtn,function()
            ViewMgr.open("luckEquipOBJ")
        end)
    end
end
