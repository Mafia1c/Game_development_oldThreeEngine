...
function Bag.main(page)
    ...
    GUI:addOnClickEvent(Bag._ui["EquipTuJianBtn"],function ()
        SendMsgCallFunByNpc(112, "EquipTuJian","upEvent",1)
    end)
    SendMsgCallFunByNpc(112, "EquipTuJian","flushRedData")
    -- 存入英雄背包
...
function Bag.OnUpdateGold(data)
    ...
    if not data or data.id == 2 then           -- 元宝
        GUI:Text_setString(Bag._ui.yuanbao, SL:GetMetaValue("ITEM_COUNT", 2))
    end
    if not data or data.id == 4 then           -- 绑定元宝
        GUI:Text_setString(Bag._ui.bind_yuanbao, SL:GetMetaValue("ITEM_COUNT", 4))
    end
    if not data or data.id == 5 then           -- 金刚石
        GUI:Text_setString(Bag._ui.jgs, SL:GetMetaValue("ITEM_COUNT", 5))
    end
    if not data or data.id == 21 then          -- 绑定金刚石
        GUI:Text_setString(Bag._ui.bind_jgs, SL:GetMetaValue("ITEM_COUNT", 21))
    end
    if not data or data.id == 7 then           -- 灵符
        GUI:Text_setString(Bag._ui.lingfu, SL:GetMetaValue("ITEM_COUNT", 7))
    end
    if not data or data.id == 20 then          -- 绑定灵符
        GUI:Text_setString(Bag._ui.bind_lingfu, SL:GetMetaValue("ITEM_COUNT", 20))
    end
end
...
function Bag.OnInitSysBtnEvent()
    local setData  = {}
    setData.index = 2                     -- 物品Index
    setData.look  = true                  -- 是否显示tips
    setData.bgVisible = false             -- 是否显示背景框
    setData.count = 1                     -- 物品数量
    GUI:ItemShow_updateItem(Bag._ui["ItemShow_1"], setData)
    setData.index = 4
    GUI:ItemShow_updateItem(Bag._ui["ItemShow_2"], setData)
    setData.index = 5
    GUI:ItemShow_updateItem(Bag._ui["ItemShow_3"], setData)
    setData.index = 21
    GUI:ItemShow_updateItem(Bag._ui["ItemShow_4"], setData)
    setData.index = 7
    GUI:ItemShow_updateItem(Bag._ui["ItemShow_5"], setData)
    setData.index = 20
    GUI:ItemShow_updateItem(Bag._ui["ItemShow_6"], setData)
    for i = 1, 6 do
        GUI:setScale(Bag._ui["ItemShow_"..i], 0.65)
    end
end

function Bag.flushRed(data)
    if type(data) == "string" and string.find(data, "T_equip_tujian_red_num") then
        if GameData.GetData("T_equip_tujian_red_num") and GameData.GetData("T_equip_tujian_red_num") > 0 then
            if  Bag._ui.tujianbtn_red == nil then
                Bag._ui.tujianbtn_red = SL:CreateRedPoint(Bag._ui.EquipTuJianBtn,{x=10,y=10}) 
            end
        else
            if Bag._ui.tujianbtn_red then
                GUI:removeFromParent(Bag._ui.tujianbtn_red)
            end
        end
    end
end
