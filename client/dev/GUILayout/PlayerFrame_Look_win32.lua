...
function PlayerFrame_Look.main(data)
    ...
    PlayerFrame_Look.OnInitSysBtnEvent()
end
...
function PlayerFrame_Look.OnInitSysBtnEvent()
    -- 血脉
    local xm_Button = PlayerFrame_Look._ui.Xm_Button
    if xm_Button then
        GUI:addOnClickEvent(xm_Button,function()
            -- ViewMgr.open("xuemaiOBJ")
             SendMsgClickSysBtn("_lookEquip#OtherSysFunc#lookPlayerInfo#5")
        end)
    end

    -- 专属
    local Zs_Button = PlayerFrame_Look._ui.Zs_Button
    if Zs_Button then
        GUI:addOnClickEvent(Zs_Button,function()
            SendMsgClickSysBtn("_lookEquip#OtherSysFunc#lookPlayerInfo#1")
        end)
    end
    local is_show =false
    for i=95,99 do
        if SL:GetMetaValue("L.M.EQUIP_DATA",i) ~= nil then
            is_show = true
        end
    end
    -- 龙神
    local Ls_Button = PlayerFrame_Look._ui.Ls_Button
    if Ls_Button then
        GUI:addOnClickEvent(Ls_Button,function()
            SendMsgClickSysBtn("_lookEquip#OtherSysFunc#lookPlayerInfo#6")
        end)
        GUI:setVisible(PlayerFrame_Look._ui.Ls_Button,is_show)  
    end
