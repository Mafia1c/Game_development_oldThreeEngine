...
function MainNear.main()
    ...
    local panel = GUI:GetWindow(nil, "MainNearBtnLayer")
    local mainPanel = GUI:getChildByName(panel, "Main_Panel")
    local panelPos = GUI:getPosition(mainPanel)

    -- 显示适配
    GUI:setPosition(Panel_1, panelPos.x - pSize.width * anchor.x, panelPos.y + pSize.height)
...
function MainNear.CreateEnemyCell(parent, data)
    ...
    GUI:addOnClickEvent(layout, function(sender)
		...
		if not SL:GetMetaValue("ACTOR_IS_PLAYER", actorID) then
			--SL:RequestLaunchSkill(0)
			SL:SetMetaValue("SELECT_TARGET_ID", actorID)
		end
