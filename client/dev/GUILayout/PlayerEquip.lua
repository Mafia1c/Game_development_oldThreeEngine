...
function PlayerEquip.ResetEquipPanelState(data)
    ...
    if isShowItem and not isInTable({87,88,89,90,91,92,93,94}, pos) then
        local itemNode = PlayerEquip._ui["Node_" .. pos]
...
function PlayerEquip.OnInitSysBtnEvent()
    local wx_btn = PlayerEquip._ui.Wx_Btn
    local animal = PlayerEquip._ui.Image_box --#region 首饰盒
    local magicRingBtn = PlayerEquip._ui.magicRingBtn --#region 上古魔戒
    local FeiJianBtn = PlayerEquip._ui.FeiJianBtn --#region 上古魔戒
    if wx_btn then
        GUI:addOnClickEvent(wx_btn, function()
            ViewMgr.open("WxzlEquipOBJ")
        end)
    end
    --#region 上古魔戒
    if magicRingBtn then
        GUI:addOnClickEvent(magicRingBtn,function()
            ViewMgr.open("magicRingEquipOBJ")
        end)
    end
    --#region 飞剑
    if FeiJianBtn then
        GUI:addOnClickEvent(FeiJianBtn,function()
            ViewMgr.open("FeiJianEquipObj")
        end)
    end
    if animal then
        GUI:addOnClickEvent(animal, function()
            SL:JumpTo(6)
        end)
    end
    --#region 境界effect显示
    local layerTab = {["初识"]=15243,["感知"]=15246,["不惑"]=15240,["洞玄"]=15245,["知命"]=15250,["天启"]=15248,["无距"]=15249,["涅槃"]=15247,["不朽"]=15241,["超凡"]=15242,["大宗师"]=15244
    ,["陆地仙人"]=46140,["至尊天人"]=46141,["圣灵道人·三生"]=46144,["圣灵道人·六道"]=46145,["圣灵道人·九天"]=46146,["圣灵道人·化神"]=46147}
    local titleTab = SL:GetMetaValue("TITLES")
    removeOBJ(PlayerEquip._ui.layerEffect)
    for index, value in ipairs(titleTab) do
        if layerTab[SL:GetMetaValue("ITEM_NAME", value["id"])] then
            GUI:Effect_Create(PlayerEquip._ui.Panel_1,"layerEffect",8,472,0,layerTab[SL:GetMetaValue("ITEM_NAME", value["id"])])
            break
        end
    end
    --#endregion
