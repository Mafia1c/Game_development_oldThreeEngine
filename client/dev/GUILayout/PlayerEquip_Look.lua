...
function PlayerEquip_Look.main(data)
    ...
    PlayerEquip_Look.OnInitSysBtnEvent()
end
...
function PlayerEquip_Look.OnInitSysBtnEvent()
    local wx_btn = PlayerEquip_Look._ui.Wx_Btn
    local animal = PlayerEquip_Look._ui.Image_box --#region 首饰盒
    local magicRingBtn = PlayerEquip_Look._ui.magicRingBtn --#region 上古魔戒
    local FeiJianBtn = PlayerEquip_Look._ui.FeiJianBtn --#region 上古魔戒
    if wx_btn then
        GUI:addOnClickEvent(wx_btn, function()
            SendMsgClickSysBtn("_lookEquip#OtherSysFunc#lookPlayerInfo#2")
        end)
    end
     --#region 飞剑
    if FeiJianBtn then
        GUI:addOnClickEvent(FeiJianBtn,function()
            SendMsgClickSysBtn("_lookEquip#OtherSysFunc#lookPlayerInfo#7")
        end)
    end
    if animal then
    ...
    if magicRingBtn then
        GUI:addOnClickEvent(magicRingBtn,function()
            SendMsgClickSysBtn("_lookEquip#OtherSysFunc#lookPlayerInfo#3")
        end)
    end
end