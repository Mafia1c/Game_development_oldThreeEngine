...
function PlayerFrame.RegisterEvent()
    ...
    SL:RegisterLUAEvent(LUA_EVENT_GAME_DATA, "PlayerFrame", PlayerFrame.flushRed)
end
...
function PlayerFrame.UnRegisterEvent()
    ...
    SL:UnRegisterLUAEvent(LUA_EVENT_GAME_DATA, "PlayerFrame")
end

function PlayerFrame.flushRed()
    local is_show = SL:GetMetaValue("LEVEL") > 40 and (GameData.GetData("U_xuemai_red") or 0) >= 1
    if PlayerFrame.red_width then
        GUI:removeFromParent(PlayerFrame.red_width)
        PlayerFrame.red_width = nil
    end 
    if is_show then
        if PlayerFrame.red_width == nil then
            PlayerFrame.red_width = SL:CreateRedPoint(PlayerFrame._ui.Xm_Button,{x=5,y=10})
        end
    end
end

function PlayerFrame.OnInitSysBtnEvent()
    -- 血脉
    local xm_Button = PlayerFrame._ui.Xm_Button
    SL:Print(xm_Button == nil)
    if xm_Button then

        GUI:addOnClickEvent(xm_Button,function()
            -- ViewMgr.open("xuemaiOBJ")
            SendMsgCallFunByNpc(0, "xuemai", "upEvent")
        end)
    end
    PlayerFrame.flushRed()
    -- 专属
    local Zs_Button = PlayerFrame._ui.Zs_Button
    if Zs_Button then
        GUI:addOnClickEvent(Zs_Button,function()
            ViewMgr.open("ExclusiveEquipOBJ")
        end)
    end
    local is_show =false
    for i=95,99 do
        if  SL:GetMetaValue("EQUIPBYPOS", i) ~= "" then
            is_show = true
        end
    end
    
    -- 龙神
    local Ls_Button = PlayerFrame._ui.Ls_Button
    if Ls_Button then
        GUI:addOnClickEvent(Ls_Button,function()
            ViewMgr.open("DragonGodEquipOBJ")
        end)

        GUI:setVisible(PlayerFrame._ui.Ls_Button, (SL:GetMetaValue("RELEVEL") or 0) >= 13 and is_show)
    end
