...
local fontPath 	= SL:GetMetaValue("WINPLAYMODE") and "fonts/font4.ttf" or GUI.PATH_FONT2
...
local function IsEquip(itemData)
	...
	if itemData.StdMode == 70 then		--允许显示称号属性
		return true
	end
...
function ItemTips.GetNodeImage(parent, param, index)
    ...
    if param.isSFX and param.res then
		...
		if param.res == 15238 then
        GUI:setContentSize(img, imgSz.width, imgSz.height + 10)
		else
...
function ItemTips.GetAttStr(itemData, diff)
	...
	if not pos and itemData.StdMode ~= 70 then		--允许显示称号属性
		return nil
	end
    local str = {} -- 添加属性提升标记
...
function ItemTips.GetSrcStr(src)
    ...
    if map and string.len(map) > 0 then
        str = str .. string.format("<font color='%s'>%s：%s</font><br>", SL:GetHexColorByStyleId(243), "地图", map)
    end
    if srcName and string.len(srcName) > 0 then
        str = str .. string.format("<font color='%s'>%s：%s</font><br>", SL:GetHexColorByStyleId(243), "出处", srcName)
    end
    if userName and string.len(userName) > 0 then
        str = str .. string.format("<font color='%s'>%s：%s</font><br>", SL:GetHexColorByStyleId(243), "角色", userName)
    end
    if time and string.len(time) > 0 then
        ...
        str = str .. string.format("<font color='%s'>%s：%s</font><br>", SL:GetHexColorByStyleId(243), "时间", timeStr)
    end
...
function ItemTips.CreateEquipPanel(data, itemData, isWear, panelInsertIndex)
    ...
    if itemData.BindInfo and string.len(itemData.BindInfo) > 0 then
        local rich_bind = GUI:RichText_Create(contentPanel, "rich_bind", 0, 0, string.format("已绑定[%s]", itemData.BindInfo), richWidth, _setFontSize or _DefaultFSize, "#FF0000", vspace, nil, fontPath)
    ...
    if itemData.Weight and itemData.Weight > 0 then
		...
		local str = string.format("类型：%s", _Item_Type[itemData.StdMode])
        local rich_weight = GUI:RichText_Create(GUI:ItemShow_GetLayoutExtra(item), "rich_weight", size.width + 10, size.height / 2 + iconMoveY, str, richWidth, _setFontSize or _DefaultFSize, "#00FFFF", vspace, nil, fontPath)
...
function ItemTips.CreateItemPanel(data, itemData)
    ...
    if itemData.BindInfo and string.len(itemData.BindInfo) > 0 then
        local rich_bind = GUI:RichText_Create(contentPanel, "rich_bind", 0, 0, string.format("已绑定[%s]", itemData.BindInfo), width, _setFontSize or _DefaultFSize, "#FF0000", vspace, nil, fontPath)
        ItemTips.PushItem(contentPanel, rich_bind)
        isAddLink = true
    end
    ...
    if isShowIcon and itemData.Bind and string.len(itemData.Bind) > 0 and SL:GetMetaValue("ITEM_IS_BIND", itemData) then
        -- ！Pos
        local rich_bind = GUI:RichText_Create(GUI:ItemShow_GetLayoutExtra(item), "rich_bind", size.width + 10, size.height + iconMoveY, "已绑定", width, _setFontSize or _DefaultFSize, "#FF0000", vspace, nil, fontPath)
        GUI:setAnchorPoint(rich_bind, 0, 1)
        GUI:RefPosByParent(rich_bind)
        local bindSize = GUI:getContentSize(rich_bind)
        maxWidth = math.max(maxWidth, bindSize.width + size.width + 20)
