...
function Notice.OnShowSystemScaleNotice(data)
	table.insert(systemNotices, data)
	Notice.CheckSystemNotice()
end

function Notice.CheckSystemNotice()
	if #noticeCells >= 3 then
		return
	end
	if not next(systemNotices) then
		return
	end

	local newData = table.remove(systemNotices, 1)
	local bgImage = GUI:Image_Create(Notice._rootSystem, "bgImage_"..math.random(1, 1000000).."_"..math.random(1, 1000000), 0, -125, "res/public/bg_hhzy_01_22.png")
	GUI:setAnchorPoint(bgImage, 0.5, 6)
	GUI:setCascadeOpacityEnabled(bgImage, true)
	noticeCells[#noticeCells + 1] = bgImage

	local bgSize = GUI:getContentSize(bgImage)
	local richText = GUI:RichTextFCOLOR_Create(bgImage, "richText", bgSize.width/2, bgSize.height/2, newData.Msg, bgSize.width + 888, 16, 0xffffff)
	GUI:setAnchorPoint(richText, 0.5, 0.5)
	GUI:setCascadeOpacityEnabled(richText, true)

	local fadeOutCallback = function()
		GUI:removeFromParent(bgImage)
		table.remove(noticeCells, 1)
		Notice.CheckSystemNotice()
	end

	local fadeAction = GUI:ActionSequence(GUI:ActionScaleTo(0, 1.5), GUI:ActionEaseBackOut(GUI:ActionScaleTo(0.3, 1)), GUI:DelayTime(1), GUI:ActionFadeOut(0.5))
	GUI:runAction(bgImage, GUI:ActionSequence(fadeAction, GUI:CallFunc(fadeOutCallback)))

	local actionTag = 1000
	local cellHeight = bgSize.height
	for k, cell in pairs(noticeCells) do
		local moveAction = GUI:ActionMoveTo(0.15, 0, cellHeight * (#noticeCells - k + 0.5))
		GUI:setTag(moveAction, actionTag)
		if cell then
			GUI:stopActionByTag(cell, actionTag)
			GUI:setPositionY(cell, cellHeight * (#noticeCells - k - 0.5))
			GUI:runAction(cell, moveAction)
		end
	end
end
...
function Notice.OnShowItemTips(info)
    ...
    local function showTips()
        ...
        if Notice.isPC then 
            ...
            local text = GUI:RichText_Create(node, "text", -40, -150, str, 1136)
