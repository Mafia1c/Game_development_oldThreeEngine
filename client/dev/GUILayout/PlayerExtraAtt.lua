...
function PlayerExtraAtt.UpdateBaseAttri()
    ...
    local gs_tab={
        [13] = {"U_gsZQ_info",0},[21] = {"U_gsBJ_info",1},[76] = {"U_gsPK_info",1}
        ,[25] = {"U_gsGJ_info",1},[77] = {"U_gsPK2_info",1},[28] = {"U_gsHSFY_info",1},[51] = {"U_gsFZBD_info",1},
        [2] = {"U_gsMFZ_info",3}
    }
    for k, v in ipairs(allList) do
        local _value = v.value

        if gs_tab[v.id] and (GameData.GetData(gs_tab[v.id][1],false) or 0) > 0 then
            if gs_tab[v.id][2]==0 then
                _value = tonumber(v.value) - (GameData.GetData(gs_tab[v.id][1],false) or 0)
            elseif gs_tab[v.id][2]==1 then
                _value = tonumber(string.sub(v.value, 1, -2)) - (GameData.GetData(gs_tab[v.id][1],false) or 0) .."%"
            elseif gs_tab[v.id][2]== 3 then
                local tab =  string.split(v.value,"/")
                _value = tonumber(tab[1]) - (GameData.GetData(gs_tab[v.id][1],false) or 0) .. "/".. tonumber(tab[2]) - (GameData.GetData(gs_tab[v.id][1],false) or 0)
            end
        end
        ...
        if v.id then
            ...
            if (config.isshow == 2 and v.value ~= 0) or config.isshow == 1 then
                ...
                if fixAttr and fixAttr.sformat and fixAttr.max then
                    if gs_tab[v.id] and (GameData.GetData(gs_tab[v.id][1],false) or 0) > 0 then
                        local gs_attr_value = (GameData.GetData(gs_tab[v.id][1],false) or 0)
                        v.value = string.format(fixAttr.sformat, v.value - gs_attr_value, fixAttr.max - gs_attr_value)
                    else
