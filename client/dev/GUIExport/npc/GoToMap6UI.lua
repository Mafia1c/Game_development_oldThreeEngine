...
function ui.init(parent, __data__, __update__)
	...
	-- Create FrameBG
	local FrameBG = GUI:Image_Create(FrameLayout, "FrameBG", 0, 0, "res/custom/npc/79wldt/bg1.png")
	GUI:Image_setScale9Slice(FrameBG, 78, 78, 171, 171)
	GUI:setContentSize(FrameBG, 780, 514)
	...
	-- Create blInfo
	local blInfo = GUI:Node_Create(FrameLayout, "blInfo", 533, 155)
	GUI:setTag(blInfo, 0)
	...
	-- Create enterMapBtn
	local enterMapBtn = GUI:Button_Create(FrameLayout, "enterMapBtn", 441, 19, "res/custom/npc/54dt/an.png")
	GUI:setContentSize(enterMapBtn, 130, 42)
	...
	-- Create closeBtn
	local closeBtn = GUI:Button_Create(FrameLayout, "closeBtn", 711, 420, "res/custom/npc/54dt/sc_close.png")
	...
	-- Create mapInfo
	local mapInfo = GUI:Node_Create(FrameLayout, "mapInfo", 523, 392)
	GUI:setTag(mapInfo, 0)

	-- Create ItemShow_6
	local ItemShow_6 = GUI:ItemShow_Create(mapInfo, "ItemShow_6", -151, -123, {index = 1, count = 1, look = true, bgVisible = true, color = 255})
	GUI:setAnchorPoint(ItemShow_6, 0.50, 0.50)
	GUI:setTag(ItemShow_6, 0)

	-- Create ItemShow_7
	local ItemShow_7 = GUI:ItemShow_Create(mapInfo, "ItemShow_7", -74, -123, {index = 1, count = 1, look = true, bgVisible = true, color = 255})
	GUI:setAnchorPoint(ItemShow_7, 0.50, 0.50)
	GUI:setTag(ItemShow_7, 0)

	-- Create ItemShow_8
	local ItemShow_8 = GUI:ItemShow_Create(mapInfo, "ItemShow_8", 3, -123, {index = 1, count = 1, look = true, bgVisible = true, color = 255})
	GUI:setAnchorPoint(ItemShow_8, 0.50, 0.50)
	GUI:setTag(ItemShow_8, 0)

	-- Create composeBtn
	local composeBtn = GUI:Button_Create(FrameLayout, "composeBtn", 583, 253, "res/public/1900000612.png")
	GUI:setContentSize(composeBtn, 104, 33)
	GUI:setIgnoreContentAdaptWithSize(composeBtn, false)
	GUI:Button_setTitleText(composeBtn, [[合成火龙珠]])
	GUI:Button_setTitleColor(composeBtn, "#009bff")
	GUI:Button_setTitleFontSize(composeBtn, 18)
	GUI:Button_titleEnableOutline(composeBtn, "#000000", 1)
	GUI:setAnchorPoint(composeBtn, 0.00, 0.00)
	GUI:setTouchEnabled(composeBtn, true)
	GUI:setTag(composeBtn, 0)
