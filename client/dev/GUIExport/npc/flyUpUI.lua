...
function ui.init(parent, __data__, __update__)
	...
	-- Create FrameBG
	local FrameBG = GUI:Image_Create(FrameLayout, "FrameBG", 0, 0, "res/custom/npc/64lh/bg1.png")
	...
	-- Create tipsText
	local tipsText = GUI:Text_Create(leftBox, "tipsText", 60, 82, 16, "#ffff00", [[轮回飞升：需要人物等级达到79级，当前转生：15转！]])
	GUI:Text_enableOutline(tipsText, "#000000", 1)
	GUI:setAnchorPoint(tipsText, 0.00, 0.00)
	GUI:setOpacity(tipsText, 0)
	GUI:setTouchEnabled(tipsText, false)
	GUI:setTag(tipsText, 0)

	-- Create rightBox
	local rightBox = GUI:Layout_Create(FrameBG, "rightBox", 820, 250, 278, 468, true)
	GUI:setAnchorPoint(rightBox, 1.00, 0.50)
	GUI:setTouchEnabled(rightBox, false)
	GUI:setTag(rightBox, 0)

	-- Create infoList1
	local infoList1 = GUI:ListView_Create(rightBox, "infoList1", 140, 376, 90, 80, 1)
	...
	-- Create text11
	local text11 = GUI:Text_Create(infoList1, "text11", 0, 56, 16, "#ffffff", [[生命加成：0%]])
	...
	-- Create text12
	local text12 = GUI:Text_Create(infoList1, "text12", 0, 30, 16, "#ffffff", [[攻击倍数：0%]])
	...
	-- Create infoList2
	local infoList2 = GUI:ListView_Create(rightBox, "infoList2", 140, 226, 90, 80, 1)
	...
	-- Create text21
	local text21 = GUI:Text_Create(infoList2, "text21", 0, 56, 16, "#00ff00", [[生命加成：10%]])
	...
	-- Create text22
	local text22 = GUI:Text_Create(infoList2, "text22", 0, 30, 16, "#00ff00", [[攻击倍数：25%]])
	...
	GUI:Button_setTitleText(upBtn, [[轮回飞升]])
