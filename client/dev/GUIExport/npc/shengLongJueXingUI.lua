...
function ui.init(parent, __data__, __update__)
	...
	-- Create Image_1
	local Image_1 = GUI:Image_Create(FrameLayout, "Image_1", 0, 0, "res/custom/npc/47jx/bg.png")
	...
	-- Create oneList
	local oneList = GUI:ListView_Create(Image_1, "oneList", 74, 47, 119, 435, 1)
	...
	-- Create RichText_1
	local RichText_1 = GUI:RichText_Create(Image_1, "RichText_1", 195, 35, [[<font color='#ffff00' size='16' >提示：觉醒成功，主装备觉醒成对应的聖龍级别装备！</font><br><font color='#ffff00' size='16' >            觉醒失败，主装备不消失，只消耗附加材料和元宝！</font><br><font color='#ffffff' size='16' >            </font><font color='#00ff00' size='16' >每次觉醒失败，增加临时成功几率1%，最高提示10%(强化成功重新计算)</font>]], 600, 16, "#ffffff", 4)
	GUI:setAnchorPoint(RichText_1, 0.00, 0.00)
	GUI:setTag(RichText_1, 0)

	-- Create jueXingButton
	local jueXingButton = GUI:Button_Create(Image_1, "jueXingButton", 507, 110, "res/custom/bt_dz.png")
	...
	-- Create needitem2
	local needitem2 = GUI:ItemShow_Create(Image_1, "needitem2", 691, 338, {index = 0, count = 1, look = true, bgVisible = false, color = 255})
	GUI:setAnchorPoint(needitem2, 0.50, 0.50)
	GUI:setTag(needitem2, 0)

	-- Create needitem3
	local needitem3 = GUI:ItemShow_Create(Image_1, "needitem3", 560, 434, {index = 0, count = 1, look = true, bgVisible = false, color = 255})
	...
	-- Create targetItem
	local targetItem = GUI:ItemShow_Create(Image_1, "targetItem", 560, 251, {index = 0, count = 1, look = true, bgVisible = false, color = 255})
